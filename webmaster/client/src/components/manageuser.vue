...
export default {
    ...
    methods: {
        //勾选复选框触发
        handleSelectionChange(val) {//把选中的信息id已-拼接起来赋值给multipleSelection
        ...
        removeuser(){
            ...
            if(this.multipleSelection===''){
                ...
                this.$confirm('此操作将永久删除该信息, 是否继续?', '提示', {
                    ...
                    this.$axios.get(this.url+'/removeuser?id='+this.multipleSelection).then(res=>{
                        // console.log(res);
                        if(res.data===true){
                            //发送请求从数据库拿数据渲染页面
                            this.$axios.get(this.url+'/manageuser').then(res=>{
        ...
        changeuser(){//点击表单确定按钮提交表单信息（如果id为空在后端执行插入代码语句，如果id不为空在后端执行修改语句）
            
            //表单id和时间可以为空
            if(this.username===''||this.usertell===''||this.useremail===''||this.userjuese===''||this.userstatus===''){
                this.$message({
                    message: '请完善全部信息!',
                    type: 'warning'
                });
            }else if(!/^1[34578]\d{9}$/.test(this.usertell)){//手机验证
                this.$message.error('手机格式不合法');
            }else if(!/^([A-Za-z0-9_\-\.])+\@([A-Za-z0-9_\-\.])+\.([A-Za-z]{2,4})$/.test(this.useremail)){
                ...
                this.$axios({
                    ...
                    data:'id='+this.userid+'&name='+this.username+'&iphone='+this.usertell+'&email='+this.useremail+'&juese='+this.userjuese+'&status='+this.userstatus
                }).then(res=>{
                    console.log(res);
...
<style>
    ...
    .manageuser .el-button--mini, .el-button--mini.is-round {
        padding: 7px 5px;
    }
    .manageuser .cell{
